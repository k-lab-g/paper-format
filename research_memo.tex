...
\subsection{B1: 背景} %%% その研究テーマを選んだ理由を書く。
\begin{description}
...
\end{description}
%
\subsection{B2: 従来の研究内容} %%% その研究テーマに関連してこれまでに行われた研究を書く
\begin{description}
...
\end{description}
%
\subsection{B3: 従来での未解明点} %%% その研究テーマに関連してこれまでに行われた研究における未解明点を書く
\begin{description}
...
\end{description}
%
\subsection{B4: 目的} %%% その研究テーマを選んだ目的を書く
\begin{description}
...
\subsection{C1: 使用した手段(簡潔)} %%% その目的を実現するために使用した手段を簡潔に書く
\begin{description}
...
\end{description}
%
\subsection{C2: 使用した手段(詳細)} %%% その目的を実現するために使用した手段を詳細に書く
\begin{description}
...
\subsection{D1: 得られた事実(簡潔)} %%% その使用した手段により得られた事実を簡潔に書く
\begin{description} 
...
\end{description}
%
\subsection{D2: 得られた事実(詳細)} %%% その使用した手段により得られた事実を詳細に書く
\begin{description}
...
%SIK方式、M-ary直交変調方式および提案方式のシンボル誤り率を評価した。その結果、提案方式のシンボル誤り率はM-ary直交変調方式とSIK方式の中間であることがわかった。提案方式は、符号間干渉が0ではない系列も使用しており、その誤り率の影響を受けるため、M-ary直交変調よりもビット誤り率特性が低下する。
% （メモ）シンボル誤り率はM-ary直交変調より悪くなるが、情報伝送効率は提案方式が最も良いことがわかったので、本論文では、情報伝送効率向上法を目的とする。シンボル誤り率については、議論しないこととする。
%
...
We also compared the proposed scheme with the SIK scheme and the M-ary orthogonal modulation scheme, as shown in Fig. \ref{fig:graph-cmp-efficiency}. Consequently, we found that the data transmission effieiency of the proposed scheme is higher than those of the SIK scheme and the M-ary orthogonal modulation scheme, which shows that the effect of increasing the number of seqences is higher than the influence of the inter symbol interference.
 \begin{itemize}
%  \item The proposed scheme achieves better symbol error rate performance than the SIK scheme. However, the symbol error rate of the proposed scheme is worse than that of the M-ary orthogonal modulation scheme. 
...
\end{figure}
%
\section{E: どのように従来及び今回の研究結果が対比されましたか?} %%% 従来の研究結果と今回得られた結果を比較する
\begin{description} 
...
\subsection{F1: 得られた結論(簡潔)} %%% データを論理的に分析して得られた結論を簡潔に書く
\begin{description}
...
\end{description}
%
\subsection{F2: 得られた結論(詳細)} %%% データを論理的に分析して得られた結論を詳細に書く
\begin{description}
...
\end{description}
%
\section{G: 今回の研究から浮かびあがった今後の課題がありましたか?} %%% 浮かび上がった問題点を今後の課題として提案する。
\begin{description} 
 \item[和文] 今後の課題として、
\begin{itemize}
...
% In future, we should evaluate the performance in the condition which is close to the real environment: 
 \begin{itemize}
